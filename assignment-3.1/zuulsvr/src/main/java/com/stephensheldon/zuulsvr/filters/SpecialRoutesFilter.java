...
import com.stephensheldon.zuulsvr.model.AbTestingRoute;
...
public class SpecialRoutesFilter extends ZuulFilter {
    ...
    //The helper variable is an instance variable of type ProxyRequestHelper class. This is a
    //Spring Cloud class with helper functions for proxying service requests.
    ...
    private AbTestingRoute getAbRoutingInfo(String serviceName){
        ...
        try {
            ...
            //If the routes services doesn’t find a record (it will return a 404 HTTP Status Code),
    ...
    public Object run() {
        ...
        //See comments to getAbRoutingInfo(...)
        AbTestingRoute abTestRoute = getAbRoutingInfo( filterUtils.getServiceId() );

        // The useSpecialRoute() method will take the weight of the route, generate a random
        //number, and determine if you’re going to forward the request onto the alternative service.
        if (abTestRoute!=null && useSpecialRoute(abTestRoute)) {
            //If there’s a routing record, build the full URL (with path) to the service
            //location specified by the specialroutes service.
